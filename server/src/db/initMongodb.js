...
const initMongoDB = async () => {
  ...
  try {
    ...
    console.log("✅ | MongoDB | MoBngoDB'ye başarıyla bağlandı!");

    // Bağlantı event listener'ları
    mongoose.connection.on("error", (err) => {
      console.error("❌ | MongoDB |  MongoDB bağlantı hatası:", err);
    });

    mongoose.connection.on("disconnected", () => {
      console.log("⚠️ | MongoDB |   MongoDB bağlantısı kesildi");
    });
  } catch (error) {
    console.error("❌ | MongoDB |   MongoDB bağlantı başarısız:");
    console.error("Hata mesajı:", error.message);

    // Yaygın hata türlerini kontrol et
    if (error.message.includes("authentication failed")) {
      console.error(
        "🔑 | MongoDB | Kimlik doğrulama hatası: Kullanıcı adı veya şifre yanlış"
      );
    } else if (error.message.includes("serverSelectionTimeoutMS")) {
      console.error(
        "⏰ | MongoDB | Sunucu bağlantı zaman aşımı: Ağ bağlantısını kontrol edin"
      );
    } else if (error.message.includes("getaddrinfo ENOTFOUND")) {
      console.error("🌐 | MongoDB | DNS hatası: MongoDB URL'i kontrol edin");
    }
